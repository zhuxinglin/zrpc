...
// 函数托离终端
int CDaemon::DoDaemon(void (*Dmain)(int, const char **), int argc, const char **args, void(*Call)(int), int iMonitor)
{
    if (iMonitor < 0 || iMonitor > 2)
    {
        fprintf(stderr, "iMonitor value:(0, 1, 2)\n");
        return -1;
    }
    assert(args != nullptr);
