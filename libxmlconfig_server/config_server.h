...
private:
    int getConnectConfig(std::string& sMysqlConfig);
...
private:
    ...
    int InitQuery(CControllerBase* pController, std::string* pMessage);

private:
    void WriteError(CControllerBase* pController, int iCode, std::string sMsg);
    int CheckAdd(CControllerBase* pController, std::string* pMessage, dao::XmlConfigTable* pConf);
    const char* getParam(const char* pszParam, const char* pszKey, std::string& sStr);
    void splitKey(std::string& sKey1, std::string& sKey2, std::string& sKey3, std::string& sKey4);
    void getQueryParam(CControllerBase* pController, std::string* pMessage, std::vector<std::string>& vParam);
